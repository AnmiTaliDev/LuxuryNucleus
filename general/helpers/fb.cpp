...
VGA::vga_text_fb *VGA_text_instance;

bool Helpers::FB::VGA_text::init()
{
    Logging::info("[Helpers/FB/VGA]: initializing VGA text driver..");
    static VGA::vga_text_fb vga_fb;
    VGA_text_instance = &vga_fb;
    if (!VGA_text_instance->init)
        Logging::err("[Helpers/FB/VGA]: failed to initialize VGA text driver");
